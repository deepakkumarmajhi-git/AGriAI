...
export async function GET(req: Request) {
  ...
  if (q) {
    filter.$or = [
      ...
      { preferredBuyerType: { $regex: q, $options: "i" } },
      { location: { $regex: q, $options: "i" } },
    ];
  }

  const listings = await WasteListing.find(filter).sort({ createdAt: -1 }).limit(100).lean();
  ...
  return Response.json({
    ...
    listings: listings.map((x: any) => ({
      ...
      preferredBuyerType: x.preferredBuyerType || "Recycler",
      status: x.status,
...
export async function POST(req: Request) {
  ...
  if (!userId || !sellerName || !wasteType || !quantity || !pricePerUnit || !location) {
    return Response.json({ error: "Missing required fields" }, { status: 400 });
  }

  const saved = await WasteListing.create({
    userId,
    sellerName,
    sellerPhone,
    wasteType,
  ...
  return Response.json({
    ...
    listing: {
      ...
      wasteType: saved.wasteType,
      quantity: saved.quantity,
      unit: saved.unit,
      pricePerUnit: saved.pricePerUnit,
      location: saved.location,
      description: saved.description,
      preferredBuyerType: saved.preferredBuyerType,
      status: saved.status,
